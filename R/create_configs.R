#' @title Change the state level of a Prey-type agent
#' @author Marina Papadopoulou (email = "marina.vp@outlook.com", ORCID = "0000-0002-6478-8365")
#' @description Replaces the existing values of the parameters in the configuration of a state
#' with the ones from the input vector
#' @param skchange a list of lists, a state from the config file of a prey-type agent
...
#' @title Change the config of a Prey-type agent
#' @author Marina Papadopoulou (email = "marina.vp@outlook.com", ORCID = "0000-0002-6478-8365")
#' @description Replaces the existing values of the parameters in the config
#' with the ones from the input vector
#' @param conf_prey a list of lists, the config file of a prey-type agent
#' @param new_params a named vector of all values to be replaced, (a line
#' of the output dataframe of the create_params function
  ...
  for (k in 1:states_size(conf_prey))
  {
    ...
    if (skchange$name == 'multi_state') {
      ...
      for (sub_ss in 1:length(skchange$sub_states)) {
        ...
        skchange$sub_states[[sub_ss]] <- sub_skchange
      }
    } else {
        skchange <- config_change_state(skchange, new_params)
    }
...
#' @title Change the config of a Predator-type agent
#' @author Marina Papadopoulou (email = "marina.vp@outlook.com", ORCID = "0000-0002-6478-8365")
#' @description Replaces the existing values of the parameters in the config
#' with the ones from the input vector
#' @param conf_pred a list of lists, the config file of a predator-type agent
#' @param new_params a named vector of all values to be replaced, (a line
#' of the output dataframe of the create_params function
#' @return the config file (list of lists) with the changed parameters
